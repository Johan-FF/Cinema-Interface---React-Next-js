...
export default function NavBar({ changeCurrentPane }: props) {
  ...
  return (
    <nav className={`z-50 absolute left-0 top-0 transition-all duration-500 h-[100%] ${viewMenu?'w-[100%]':' w-0 md:w-[20%]'} bg-tertiary shadow-2xl shadow-black`}> 
      ...
      <button onClick={() => {setViewMenu(!viewMenu)}} className={`z-100 md:hidden transition-all duration-1000 absolute top-[3%] left-[5%] h-[5vh] w-[5vh] bg-tertiary hover:bg-tertiary-opacity rounded-2xl ${viewMenu?'rotate-180':''}`}>
