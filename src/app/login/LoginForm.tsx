...
export default function LoginForm({ sendMessage, inputs }: props) {
  ...
  const validatePassword = () => {
    ...
    setInvallidPassword('conPassword' in values && (values.password != values.conPassword))
  }
    ...
    <form 
      ...
      onSubmit={handleSubmit(onSubmit)}>
      {
        inputs.map((value, index) => {
          return (
            <>
              <label key={index} htmlFor={value.id} className="label">{value.content}:</label>
              <input key={index*2+1} type="text" id={value.id} placeholder={value.placeHolder} className="text-input" {...register(value.id)} onChange={value.id==='conPassword' ? validatePassword : ()=>{}}/>
              {errors[value.id as keyof userData]?.message && <label className='label-error'>{errors[value.id as keyof userData]?.message}</label>}
            </>
          )
        })
      }
      {
        invalidPassword ? <label className='label-error'>La contraseña no coincide</label> : ''
      }
